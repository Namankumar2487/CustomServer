...
module.exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    sendOTP(email);
    res.status(201).json({ status: "SUCCESS", message: "OTP Sent Successfully" });
  } catch (err) {
    console.log(err);
    const errors = handleErrors(err);
...
module.exports.resetPassword = async (req, res, next) => {
  try {
    ...
    let data = PassResetModel.find({ email })
    await PassResetModel.find({ email }).then((data) => {
      console.log(data)
      if (data[0].expiresAt < Date.now()) {
        PassResetModel.deleteOne({ email }).then(() => { res.status(400).json({ status: "Failed", message: "Token Expired" }); });
      }
      else {
        console.log("OTP" + otp)
        ...
        if (otp == data[0].resetOTP) {
          ...
          process.nextTick(() => {encryptedpassword = encryptpass(password)})
          console.log(encryptedpassword)
          UserModel.updateOne({ email: email }, { password: encryptedpassword }).then(() => {
            PassResetModel.deleteOne({ email }).then(() => {
              res.status(400).json({ status: "Success", message: "Password Reset Successful" });
            })
          }).catch(() => { res.status(400).json({ status: "Failed", message: "email not in database" }); })
        }
        else {
...
const encryptpass = async (password) => {
  ...
  return encryptedpassword = await bcrypt.hash(password, salt);
}